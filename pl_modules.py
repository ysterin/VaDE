...
if __name__ == '__main__':
    model = PLVaDE(n_neurons=[784, 512, 512, 2048, 10], k=10, lr=2e-3, pretrain_lr=3e-4, covariance_type='full', 
                   batch_size=2**8, pretrain_epochs=10, do_pretrain=False,
                #    pretrained_model_file="AE clustering/5wn5ybl3/checkpoints/epoch=69-step=16449.ckpt", 
                #    init_gmm_file='saved_gmm_init/5wn5ybl3/gmm-full-acc=0.95.pkl',
                   multivariate_latent=False, rank=5, device='cuda', dataset='mnist')
    base_dm = MNISTDataModule(dataset='mnist', bs=256)
    logger = pl.loggers.WandbLogger(project='VADE')
    # profiler = pl.profiler.AdvancedProfiler(output_filename='profiler_log_4096.txt')
    trainer = pl.Trainer(gpus=1, logger=logger, progress_bar_refresh_rate=50, max_epochs=3,
                        callbacks=[ClusteringEvaluationCallback(ds_type='all'), PretrainingCallback(20, 3e-4)], 
                        log_every_n_steps=5)
